...
tmux new-session -d -x $(tput cols) -y $(tput lines) -s $TMUX_DEFAULT_SESSION_NAME -n org "sh $HOME/.when/update_calendar.sh"
tmux split-window -v -t $TMUX_DEFAULT_SESSION_NAME "sh $HOME/.task/update_task.sh"
